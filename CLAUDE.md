...
- **UI**: Panda CSS (`styled-system/*`, tokens/recipes in `panda.config.ts`, `preflight: false` — see the note there) + Ark UI headless parts + `lucide-react` icons. Site primitives live in `src/components/ds/` (Button, Text, Card, Badge, Modal/BareModal, Menu, Tabs, Drawer, BottomSheet, Slider, RadioGroup, Pager, toast…), form controls in `src/components/ui/inputs/`, the admin console's own kit in `src/components/admin/ui.tsx`. No Tailwind, no CSS-in-JS runtime.
- **Typography**: one font (Inter, loaded in `src/app/layout.tsx`) and nine size roles defined as `textStyles` in `panda.config.ts`. Components write `textStyle: "ui"` (or `<Text size="ui">`), never `fontSize` / `lineHeight` / `letterSpacing` / `fontFamily` — ESLint rejects those. Money uses `fontVariantNumeric: "tabular-nums"`, not a mono font. Rules, roles and how to change any of it: `TYPOGRAPHY.md`.
- **MUI is gone** (removed 2026-09-07 → 2026-09-13, see `MIGRATION-PANDA.md` for the styling rulebook and `HANDOFF.md` for the log). Never add `@mui/*` or `@emotion/*` back; new or rewritten UI uses the `ds` kit, `css()`/`cva()` from `styled-system/css`, Ark UI for anything interactive, and lucide icons. `globals.css` keeps bare `p/h*/ul { margin: 0 }` and `a { color: inherit }` rules on purpose: they beat every Panda class, so margins on those elements go on a wrapping div and link colours need `!important` or the ds `Link` (MIGRATION-PANDA.md has the details).
