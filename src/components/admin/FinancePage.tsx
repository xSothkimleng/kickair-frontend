...
const kpiNum = css({ textStyle: "title", fontWeight: 600, fontVariantNumeric: "tabular-nums", mt: "6px" });
  ...
  const inner = (
    <div className={row({ gap: 3 })}>
      ...
      <div>
        ...
        <p className={text({ size: "meta", tone: 3 })}>{t.user?.email ?? ""}</p>
      </div>
    </div>
  );
      ...
      {tab === "payouts" ? (
        <>
          <div className={cx(row({ between: true }), css({ mb: "14px" }))}>
            ...
            {pf === "pending" && prow.length ? <span className={text({ size: "meta", tone: 3 })}>Oldest first</span> : null}
          </div>
          <Panel>
        ...
        <>
          <div className={cx(row({ between: true }), css({ mb: "14px" }))}>
            <div className={row({ gap: 2 })}>
              <span className={text({ size: "meta", tone: 2 })}>Type</span>
            ...
            {txns.data ? <span className={text({ size: "meta", tone: 3 })}>{txns.data.data.meta.total.toLocaleString("en-US")} transactions</span> : null}
          </div>
          <Panel>
